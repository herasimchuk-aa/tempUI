...
import { SET_NODES, SET_SELECTED_NODE_IDS, SET_ACTUAL_NODE_INFO, SET_NODE_HEADING, SET_CONNECTIVITY_HEADING } from '../actions/nodeAction';
import { nodeHead, connectivityHead } from '../consts';
...
const inititalState = I.fromJS({
    ...
    'connectivityHeadings': I.fromJS(connectivityHead)
})
...
export default function nodeReducer(state = inititalState, action) {
    switch (action.type) {
        ...
        case SET_NODE_HEADING:
            ...
            return state
        case SET_CONNECTIVITY_HEADING:
            state = state.set('connectivityHeadings', action.payload)
            return state
        default:
            return state
    }
